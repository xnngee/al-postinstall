...
configure_de() {
    ...
    sudo fly-wmfunc FLYWM_UPDATE_VAL TaskbarBoldFont "Inter Display-9:bold"
    ...
    sudo fly-wmfunc FLYWM_UPDATE_VAL TaskbarFont "Inter Display-9:normal"
    sudo fly-wmfunc FLYWM_UPDATE_VAL TaskbarLangFont "Inter Display-10:normal"
    sudo fly-wmfunc FLYWM_UPDATE_VAL TitleFont "Inter Display-9:normal"
...
cds_setup() {
# start script
cat << EOF >> /home/cds/start-cds.sh
#!/bin/bash
xset -dpms
xset s off
# cds_url="https://map.cdsvyatka.com/avia/index3.php"
cds_url="0x0.st"
while true; do
    rm -rf ~/.{config,cache}/chromium/
    chromium-browser --kiosk --no-first-run --disable-translate \$cds_url
    sleep 1s
done
EOF
chown cds:cds /home/cds/start-cds.sh
chmod 755 /home/cds/start-cds.sh
chmod +x /home/cds/start-cds.sh
...
while true; do
    ...
    echo ""
    echo "1. Auto PostInstall (by 2 to 6 functions)"
